...
(() => {
  ...
  function decision(){
    if(hasExistingUnitProgress()) return {allow:true,source:"existing_unit_progress",pathway:"legacy_or_active",message:"Existing Unit 01 progress preserved."};
    ...
    if(diagnostic?.route==="unit_0_refresher"){
      return {allow:false,source:"readiness_gateway",pathway:"supported_42_week",destination:"../../foundations/",title:"Science Foundations comes first",message:"Your readiness profile recommends the six-week Science Foundations Refresher before Official Unit 1. Complete the bridge and its ≥80% mastery gateway, then Unit 1 opens directly."};
    }

    if(foundation&&foundation.route!=="advance"){
      return {allow:false,source:"science_foundations",pathway:"supported_42_week",destination:"../../foundations/mastery-assessment.html",title:"Finish the Foundations mastery gateway",message:"Your Science Foundations record is not yet cleared for Unit 1. Complete targeted corrections or reassessment until the ≥80% mastery rule and essential-strand requirement are met."};
    }

    return {allow:false,source:"none",pathway:"unassigned",destination:"../../diagnostic/readiness-diagnostic.html",title:"Begin with the Science 9 Readiness Gateway",message:"Before Official Unit 1, complete the readiness diagnostic. Students who are ready enter the 36-week core course; students needing reinforcement receive the six-week refresher and a 42-week supported pathway."};
  }
  ...
  function recordAdmission(d){
    ...
    root.pathway=d.pathway==="supported_42_week"?"Supported 42-week":"Core 36-week";
