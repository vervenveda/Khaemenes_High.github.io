...
function learnerClaims(){if(cfg.mode!=="formal"||!cfg.learnerId)return {};const all=readJSON(CLAIMS_KEY,{});return all[cfg.learnerId]||{}}
function claimMatches(id,e){const c=learnerClaims()[id];return Boolean(c&&e&&c.submittedAt===e.submittedAt&&Number(c.score)===Number(e.score)&&Number(c.total)===Number(e.total))}
function claimedEvidence(){const out={};if(cfg.mode!=="formal"||!cfg.learnerId)return out;for(const m of MAP){const e=rawEvidence(m);if(e&&claimMatches(m.id,e))out[m.id]=e}return out}
function mergeFormal(cumulative){
  ...
  if(typeof active==="function"){const s=active();if(s)s.cumulative={...(s.cumulative||{}),...cumulative}}
}
function claimLatest(id){
  ...
  writeJSON(CLAIMS_KEY,all);mergeFormal(claimedEvidence());if(typeof renderAssessments==="function")renderAssessments();return true;
}
function sync(){const cumulative=claimedEvidence();mergeFormal(cumulative);return cumulative}
...
if(priorAssess){
  renderAssessments=function(){
    ...
    const mapping=[["Units 01–04 Cumulative Test","units01_04"],["Units 05–07 Cumulative Test","units05_07"],["Units 08–09 Cumulative Test","units08_09"],["Comprehensive Final Examination","final"]];
    for(const [heading,id] of mapping){
      const card=cards.find(x=>x.querySelector("h3")?.textContent.trim()===heading);if(!card)continue;
      const e=c[id],latest=rawEvidence(MAP.find(x=>x.id===id)),status=document.createElement("div");status.className="notice";
      if(e){status.textContent="Learner-attributed objective evidence: "+e.percent+"% · "+(e.passed?"80% mastery reached":"mastery not yet reached")+" · "+e.attempts+" attempt(s)."}
      ...
      }else if(latest){status.textContent="A browser-local assessment result exists, but preview mode cannot attach it to a formal learner record."}
      else{status.textContent="No learner-attributed submitted objective score is available yet."}
