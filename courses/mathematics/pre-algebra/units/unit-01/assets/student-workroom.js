...
function resultBest(result){
  if(!result||typeof result!=="object")return 0;
  const attempts=Array.isArray(result.attempts)?result.attempts:[];
  return Math.max(
    Number(result.best_percent)||0,
    Number(result.percent)||0,
    Number(result.latest_percent)||0,
    ...attempts.map(attempt=>Number(attempt?.percent)||0)
  );
}
...
function bestFor(id){
  const p=progress();
  return Math.max(Number(p.lessonBestScores?.[id])||0,Number(p.lessonScores?.[id])||0);
}
...
function addStyles(){
  ...
  style.textContent=`
    ...
    .daily-assignment-card .daily-kicker{margin:0 0 10px;color:var(--gold2,#8c6b2d);font-size:12px;font-weight:700;letter-spacing:.18em;text-transform:uppercase}
    ...
    body.student-workroom .lesson-nav #nextLesson[data-locked="true"],body.student-workroom .lesson-nav #nextLesson[aria-disabled="true"]{display:none!important}
    ...
    .daily-finish-actions .btn{min-height:48px}
    .daily-finish-actions #submitPractice{min-width:240px}
    .daily-recovery,.daily-success{width:min(100%,920px);margin:18px auto 0;padding:22px;border:1px solid var(--line,#d8d8d2);border-radius:11px;background:var(--panel,#fff);text-align:left}
    .daily-recovery{border-left:4px solid var(--gold,#b48b45)}
    .daily-success{border-left:4px solid var(--green,#4f7657)}
    .daily-recovery h3,.daily-success h3{margin:0 0 8px;font:400 25px/1.12 Georgia,"Times New Roman",serif}
    .daily-recovery p,.daily-success p{color:var(--muted,#596269)}
    .daily-recovery ol{display:grid;gap:8px;padding-left:22px}
    .daily-recovery li{padding:8px 10px;border:1px solid var(--line,#d8d8d2);border-radius:7px;background:var(--bg2,#f6f4ef)}
    .daily-support-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}
    .daily-success .daily-assignment-cta{width:auto;min-height:52px;font-size:16px;padding-inline:18px}
    @media(max-width:700px){
      .daily-assignment-hub{padding:24px 0 34px!important}
      .daily-assignment-card{padding:22px 16px}
      .daily-assignment-cta{width:100%}
      .daily-finish-actions,.daily-support-actions{display:grid;grid-template-columns:1fr}
      .daily-finish-actions .btn,.daily-support-actions .btn,.daily-support-actions a{width:100%}
      body.student-workroom .site-header #studentHomeButton{min-height:40px}
    }
    @media print{
      .daily-assignment-hub,.daily-lesson-status,.daily-finish-panel,.daily-recovery,.daily-success{display:none!important}
    }
  `;
...
function assignmentState(){
  ...
  if(current){
    const best=bestFor(current.id);
    return{
      kind:"lesson",
      title:`Lesson ${current.number} · ${current.title}`,
      message:best>0?`Continue today’s lesson. Your best demonstrated score is ${best}%; ${MIN}% is required for mastery.`:`This is today’s assignment. Complete the lesson and submit the practice when you are ready.`,
      href:current.file,
      label:best>0?`Continue Lesson ${current.number}`:`Begin Lesson ${current.number}`,
      mastered,
  ...
  if(masteryBest()<MIN){
    return{
      ...
      message:`All ${LESSONS.length} daily lessons are mastered. Complete the Unit 1 mastery check at ${MIN}% or higher before the next unit appears.`,
      href:map.assessment?.file||"assessment/mastery-check.html",
      label:"Take Unit 1 Mastery Check",
      mastered,
      total:LESSONS.length
    };
  }
  return{
    ...
    message:`You have mastered all ${LESSONS.length} lessons and the Unit 1 mastery check. Unit 2 is now ready.`,
    href:"../unit-02/",
...
function renderUnitHub(){
  ...
  if(!hub){
    ...
    hub.id="dailyAssignmentHub";
    hub.className="daily-assignment-hub";
  ...
  const panel=document.querySelector(".mastery-gate-panel");
  if(panel)panel.hidden=!unitReady();
}

function mentorURL(missed){
  const params=new URLSearchParams({course:"Pre-Algebra",unit:"1",lesson:String(window.LESSON_DATA?.number||""),topic:window.LESSON_DATA?.title||"",missed:missed.join(",")});
  return `https://artist1970.github.io/Archaemenes.github.io/high/?${params.toString()}`;
}
...
function ensureLessonWorkroom(){
  ...
  if(eyebrow)eyebrow.textContent=`Daily Assignment · Lesson ${lesson.number} · ${lesson.duration||""}`;
  const begin=document.querySelector(".hero .actions a[href='#learn']");
  if(begin){begin.textContent="Begin Today’s Assignment";begin.setAttribute("aria-label",`Begin daily assignment: ${lesson.title}`)}

  const learn=document.getElementById("learn");
  if(learn&&!document.getElementById("dailyLessonStatus")){
    const status=document.createElement("div");
    status.id="dailyLessonStatus";
    status.className="daily-lesson-status";
    learn.querySelector(".wrap")?.prepend(status);
  }
  renderLessonStatus();
  buildFinishPanel();
  renderOutcomeFromStoredScore(false);
}

function renderLessonStatus(){
  const lesson=window.LESSON_DATA;
  const target=document.getElementById("dailyLessonStatus");
...
function buildFinishPanel(){
  ...
  const wrap=practice?.querySelector(".wrap");
  if(!wrap||document.getElementById("dailyFinishPanel"))return;
  const panel=document.createElement("div");
  panel.id="dailyFinishPanel";
  panel.className="daily-finish-panel";
  ...
  const actions=panel.querySelector(".daily-finish-actions");
  const submit=document.getElementById("submitPractice");
  const reset=document.getElementById("resetPractice");
  const print=document.getElementById("printLesson");
  const download=document.getElementById("exportLesson");
  if(submit){submit.textContent="Submit Daily Assignment";actions.appendChild(submit)}
  if(print){print.textContent="Print Lesson";actions.appendChild(print)}
  if(download){download.textContent="Download Lesson Record";actions.appendChild(download)}
  if(reset){reset.textContent="Clear Answers";actions.appendChild(reset)}
  const score=document.getElementById("scoreMessage");
  if(score)score.insertAdjacentElement("afterend",panel);else wrap.appendChild(panel);
}

function latestAndBest(){
  const lesson=window.LESSON_DATA;
  const p=progress();
  const rawLatest=p.lessonLatestScores?.[lesson?.id]??p.lessonScores?.[lesson?.id];
  const latest=Number(rawLatest);
  return{latest:Number.isFinite(latest)?latest:null,best:lesson?bestFor(lesson.id):0};
}

function missedProblems(){
  const questions=window.LESSON_DATA?.questions||[];
  const missed=[];
  questions.forEach((question,index)=>{
    const selected=document.querySelector(`input[name="q${index}"]:checked`);
    if(!selected||Number(selected.value)!==Number(question.answer))missed.push({number:index+1,explanation:question.explanation||"Review this skill and try the problem again."});
  });
  return missed;
}

function nextLessonState(){
  const lesson=window.LESSON_DATA;
  const index=LESSONS.findIndex(item=>item.id===lesson?.id);
  const next=LESSONS[index+1];
  if(next)return{href:next.file.split("/").pop(),label:`Continue to Lesson ${next.number}`,title:next.title};
  return{href:"../assessment/mastery-check.html",label:"Take Unit 1 Mastery Check",title:"Unit 1 Mastery Check"};
}

function renderOutcomeFromStoredScore(scroll){
  const outcome=document.getElementById("dailyOutcome");
  const lesson=window.LESSON_DATA;
  if(!outcome||!lesson)return;
  const {latest,best}=latestAndBest();
  if(latest===null){outcome.innerHTML="";return}
  const missed=missedProblems();
  if(best>=MIN){
    const next=nextLessonState();
    outcome.innerHTML=`<article class="daily-success"><p class="daily-kicker">Mastery reached</p><h3>${best}% demonstrated mastery</h3><p>Your best score meets the ${MIN}% standard. The next required step is now available.</p><a class="daily-assignment-cta" href="${escapeHTML(next.href)}">${escapeHTML(next.label)}</a></article>`;
  }else{
  ...
  if(next)next.style.display=best>=MIN?"":"none";
  renderLessonStatus();
  if(scroll)outcome.scrollIntoView({behavior:"smooth",block:"center"});
}

function bindLessonOutcome(){
  const submit=document.getElementById("submitPractice");
  if(!submit||submit.dataset.dailyWorkroomBound==="true")return;
  submit.dataset.dailyWorkroomBound="true";
  submit.addEventListener("click",()=>setTimeout(()=>renderOutcomeFromStoredScore(true),0));
  window.addEventListener("storage",event=>{if(event.key===PROGRESS_KEY){renderLessonStatus();renderOutcomeFromStoredScore(false)}});
}

function init(){
  addStyles();
  if(window.LESSON_DATA){ensureLessonWorkroom();bindLessonOutcome();return}
  if(document.getElementById("lessonGrid")||document.querySelector(".mastery-gate-panel")){
    renderUnitHub();
    window.addEventListener("storage",event=>{if(event.key===PROGRESS_KEY||event.key===MASTERY_KEY)renderUnitHub()});
  }
}
