...
(() => {
  ...
  function scorePractice() {
    ...
    /*
      IMPORTANT:
      Practice completion uses the current Academy mastery threshold.
  ...
  function renderRecord() {
      ...
      if (hasScore) {
        status.textContent =
          ...
          (
            completed
              ? "Lesson completed"
              : `${CONFIG.masteryThreshold}% mastery required`
          );
      } else {
        status.textContent =
          completed
    ...
    if (button) {
      button.textContent =
        completed ? "Remove Completion Mark" : "Mark Lesson Reviewed";
