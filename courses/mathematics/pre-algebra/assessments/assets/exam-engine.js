...
(function(){
 ...
 function submit(){if(Object.keys(state.answers).length!==C.questions.length){$("status").textContent="Answer every question before submitting.";return}state.submitted=true;let correct=0;const by={};C.questions.forEach((q,i)=>{const k=`Unit ${q.unit}`;by[k]??={correct:0,total:0};by[k].total++;if(Number(state.answers[i])===q.answer){correct++;by[k].correct++}});const percent=Math.round(correct/C.questions.length*1000)/10;const threshold=Number.isFinite(Number(C.mastery_threshold))?Number(C.mastery_threshold):null;const result={assessment_id:C.id,title:C.title,correct,total:C.questions.length,percent,letter_grade:grade(percent),submitted_at:new Date().toISOString(),section_scores:by};if(threshold!==null){result.mastery_threshold=threshold;result.mastery_met=percent>=threshold}localStorage.setItem(resultKey,JSON.stringify(result));save();render();showResult(result)}
