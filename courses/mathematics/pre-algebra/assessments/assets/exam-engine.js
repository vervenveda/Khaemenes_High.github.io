/* Khaemenes Academy Exam Engine · Existing EXAM_CONFIG markup · Design v20 */
...
(function(){
 ...
 function submit(){if(Object.keys(state.answers).length!==C.questions.length){$("status").textContent="Answer every question before submitting.";return}state.submitted=true;let correct=0;const by={};C.questions.forEach((q,i)=>{const k=`Unit ${q.unit}`;by[k]??={correct:0,total:0};by[k].total++;if(Number(state.answers[i])===q.answer){correct++;by[k].correct++}});const percent=Math.round(correct/C.questions.length*1000)/10;const result={assessment_id:C.id,title:C.title,correct,total:C.questions.length,percent,letter_grade:grade(percent),submitted_at:new Date().toISOString(),section_scores:by};localStorage.setItem(resultKey,JSON.stringify(result));save();render();showResult(result)}
 function showResult(r){$("results").hidden=false;$("resultScore").textContent=r.percent+"%";$("resultTitle").textContent=`${r.correct} of ${r.total} correct · Grade ${r.letter_grade}`;const grid=$("breakdown");grid.innerHTML="";Object.entries(r.section_scores).forEach(([k,v])=>{const a=document.createElement("article");a.innerHTML=`<strong>${v.correct}/${v.total}</strong><span>${k}</span>`;grid.append(a)});$("results").scrollIntoView({behavior:"smooth"})}
