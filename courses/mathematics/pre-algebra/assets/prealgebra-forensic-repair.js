...
function bank(w,salt=0){
  ...
  if(d==="number"){
    const a=12+(n%7),b=18+(n%5),g=6,l=(a/g)*(b/g)*g;
    ...
    return [
      ...
      I(`What is the greatest common factor of ${a} and ${b}?`,g,[2,3,Math.min(a,b)],"The GCF is the greatest positive integer dividing both numbers.",S(1)),
  ...
  if(d==="percent"){
    ...
    return [
      ...
      I(`A meal costs $${40+5*(n%5)} before a ${rate}% tip. Which expression gives the tip?`,`${(40+5*(n%5))}×${rate/100}`,[`${(40+5*(n%5))}+${rate}`,`${(40+5*(n%5))}÷${rate}`,`${rate}÷${(40+5*(n%5))}`],"Convert the percent to a decimal and multiply by the pre-tip amount.",S(3)),
  ...
  if(d==="inequalities"){
    const a=3+n%5,b=8+n%6,sol=b-a,m=2+n%4;
    return [
      ...
      I(`A venue can hold at most ${40+5*(n%5)} people. Which inequality models attendance p?`,`p≤${40+5*(n%5)}`,[`p≥${40+5*(n%5)}`,`p<${40+5*(n%5)}`,`p>${40+5*(n%5)}`],"“At most” means less than or equal to the limit.",S(4))
    ];
  }
  if(d==="coordinate"){
