...
function applyWeekLocks(){
  ...
  document.querySelectorAll("[data-week]").forEach(btn=>{
    ...
    btn.onclick=()=>{
      if(!ok){
        const miss=firstMissingBefore(n);
        if(n>=19&&weeksMastered(1,18)&&midtermBest()<TARGET)return friendly("Semester II opens after the midterm reaches 80% mastery.");
        return friendly(`Week ${n} opens after ${miss?`Week ${miss}`:"the prior week"} reaches full lesson/assignment completion and 80% mastery.`);
      }
