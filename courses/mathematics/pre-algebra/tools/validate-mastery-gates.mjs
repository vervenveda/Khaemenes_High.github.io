...
for(let n=2;n<=13;n++){
  const id=String(n).padStart(2,'0'),le=read(`units/unit-${id}/assets/lesson-engine.js`),dash=read(`units/unit-${id}/assets/unit-dashboard.js`);
  must(!/percent\s*>=\s*70|>=\s*70/.test(le),`Unit ${n}: legacy 70% threshold found`);
  must(/MIN=Math\.max\(80/.test(le),`Unit ${n}: hardened 80% lesson engine missing`);
  must(/lessonBestScores/.test(le)&&/lessonAttempts/.test(le),`Unit ${n}: best-score attempt history missing`);
  must(/aria-disabled/.test(le)&&/renderNavigation/.test(le),`Unit ${n}: sequential next-lesson gate missing`);
  must(/Prior lesson requires 80%/.test(dash),`Unit ${n}: dashboard sequential lesson gate missing`);
  must(/data-review/.test(dash)&&!/data-complete/.test(dash),`Unit ${n}: dashboard review/mastery separation missing`);
  must(/allLessons\(\)/.test(dash)&&/assessment\/mastery-check\.html/.test(dash),`Unit ${n}: unit assessment must remain locked until all lessons reach mastery`);
}
for(let n=1;n<=13;n++){
