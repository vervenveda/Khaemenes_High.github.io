...
/*
  Khaemenes Open-Age Algebra II / Advanced Functions
...
.khae-a2-portrait{width:168px;height:168px;margin:auto;overflow:hidden;border:1px solid #d4b672;border-radius:50%;background:radial-gradient(circle,#fff8e6,#ead7ad)}
.khae-a2-portrait img{width:100%;height:100%;object-fit:cover;object-position:center top}
.khae-a2-copy{text-align:left}
.khae-a2-kicker{color:var(--k-burgundy);font:600 .67rem var(--k-formal);letter-spacing:.16em;text-transform:uppercase}
.khae-a2-mentor h2{margin:5px 0 8px;color:var(--k-forest);font:500 clamp(1.9rem,4vw,3.1rem)/1 var(--k-formal)}
.khae-a2-mentor p{margin:0;color:var(--k-muted)}
.khae-a2-context{margin-top:12px!important;color:var(--k-forest)!important;font-weight:700}
.khae-a2-actions{margin-top:18px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}
.khae-a2-actions a{min-height:43px;display:flex;align-items:center;justify-content:center;padding:9px 13px;border:1px solid var(--k-line);border-radius:8px;font-size:.68rem;font-weight:800;letter-spacing:.07em;text-transform:uppercase;text-decoration:none}
.khae-a2-actions .primary{color:#fff;background:var(--k-forest)}.khae-a2-actions .secondary{color:var(--k-forest);background:#fff}
.khae-a2-note{margin-top:13px!important;font-size:.78rem!important;color:#7a817d!important}
.khae-a2-beta{position:fixed;right:18px;bottom:18px;z-index:1300;min-width:112px;min-height:42px;display:flex;align-items:center;justify-content:center;padding:9px 15px;border:1px solid #cbaa64;border-radius:999px;color:#2c2618!important;background:linear-gradient(145deg,#f5e6c0,#d9b66a);box-shadow:0 11px 28px rgba(18,34,28,.16);font:600 .61rem var(--k-formal);letter-spacing:.11em;text-transform:uppercase;text-decoration:none}
@media(max-width:980px){.khae-a2-actions{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:860px){.khae-a2-mentor{grid-template-columns:1fr;text-align:center}.khae-a2-copy{text-align:center}.khae-a2-actions{grid-template-columns:1fr}}
@media(max-width:620px){.hero h1{font-size:clamp(3rem,13vw,4.8rem)!important}.khae-a2-mentor{width:min(calc(100% - 28px),1240px);padding:22px 16px}.khae-a2-beta{right:10px;bottom:10px;min-width:96px;min-height:38px;font-size:.55rem}}
@media print{.khae-a2-mentor,.khae-a2-beta{display:none!important}}
`;

const style=document.createElement("style");
style.id="khaemenes-algebra2-archaemenes-theme-v1";
...
function mentorHref(){
  ...
  const q=new URLSearchParams({course:"Algebra II",concept:"advanced algebra"});
...
function contextText(){
  const c=contextFromPage(),parts=["Algebra II"];
  if(c.week)parts.push(`Week ${c.week}`);if(c.view)parts.push(c.view);if(c.heading&&c.heading!==c.view)parts.push(c.heading);
  return parts.join(" · ");
}
function createMentor(){
  if(document.getElementById("khaeA2Mentor"))return;
  const main=document.querySelector("main");if(!main)return;
  const s=document.createElement("section");
  s.className="khae-a2-mentor";s.id="khaeA2Mentor";s.setAttribute("aria-labelledby","khaeA2MentorTitle");
  s.innerHTML=`
  <div class="khae-a2-portrait"><img src="https://artist1970.github.io/Archaemenes.github.io/assets/Archaemens-high.png" alt="Archaemenes, High School educational mentor"></div>
  <div class="khae-a2-copy">
    <div class="khae-a2-kicker">Scholar · Educational Mentor · Mathematics</div>
    <h2 id="khaeA2MentorTitle">Study Algebra II with Archaemenes.</h2>
    <p>Use Archaemenes for explanations, hints, fresh examples, function analysis, equation strategy, modelling support, and prerequisite review while keeping graded work independent.</p>
    <p class="khae-a2-context" id="khaeA2Context"></p>
    <div class="khae-a2-actions">
      <a class="primary" id="khaeA2MentorLink" href="${mentorHref()}">Open Archaemenes</a>
    ...
    <p class="khae-a2-note">Mentor boundary: Archaemenes may teach, explain, guide and review. Algebra II course code remains authoritative for assessments, mastery, records and progression.</p>
  </div>`;
  main.prepend(s);refresh();
}
function refresh(){const c=document.getElementById("khaeA2Context"),l=document.getElementById("khaeA2MentorLink");if(c)c.textContent=contextText();if(l)l.href=mentorHref()}
function addBeta(){if(document.getElementById("khaeA2Beta"))return;const a=document.createElement("a");a.id="khaeA2Beta";a.className="khae-a2-beta";a.href=BETA_URL;a.textContent="Beta Program";a.setAttribute("aria-label","Open the Verve N Veda Beta Program");document.body.appendChild(a)}
