...
.khae-calc-portrait{width:168px;height:168px;margin:auto;overflow:hidden;border:1px solid #d4b672;border-radius:50%;background:radial-gradient(circle,#fff8e6,#ead7ad)}
.khae-calc-portrait img{width:100%;height:100%;object-fit:cover;object-position:center top}
.khae-calc-copy{text-align:left}
.khae-calc-kicker{color:var(--k-burgundy);font:600 .67rem var(--k-formal);letter-spacing:.16em;text-transform:uppercase}
.khae-calc-mentor h2{margin:5px 0 8px;color:var(--k-forest);font:500 clamp(1.9rem,4vw,3.1rem)/1 var(--k-formal)}
.khae-calc-mentor p{margin:0;color:var(--k-muted)}
.khae-calc-context{margin-top:12px!important;color:var(--k-forest)!important;font-weight:700}
.khae-calc-actions{margin-top:18px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}
.khae-calc-actions a{min-height:43px;display:flex;align-items:center;justify-content:center;padding:9px 13px;border:1px solid var(--k-line);border-radius:8px;font-size:.68rem;font-weight:800;letter-spacing:.07em;text-transform:uppercase;text-decoration:none}
.khae-calc-actions .primary{color:#fff;background:var(--k-forest)}.khae-calc-actions .secondary{color:var(--k-forest);background:#fff}
.khae-calc-note{margin-top:13px!important;font-size:.78rem!important;color:#7a817d!important}
.khae-calc-beta{position:fixed;right:18px;bottom:18px;z-index:1300;min-width:112px;min-height:42px;display:flex;align-items:center;justify-content:center;padding:9px 15px;border:1px solid #cbaa64;border-radius:999px;color:#2c2618!important;background:linear-gradient(145deg,#f5e6c0,#d9b66a);box-shadow:0 11px 28px rgba(18,34,28,.16);font:600 .61rem var(--k-formal);letter-spacing:.11em;text-transform:uppercase;text-decoration:none}
@media(max-width:860px){.khae-calc-mentor{grid-template-columns:1fr;text-align:center}.khae-calc-copy{text-align:center}.khae-calc-actions{grid-template-columns:1fr}}
@media(max-width:620px){.hero h1{font-size:clamp(3rem,13vw,4.8rem)!important}.khae-calc-mentor{width:min(calc(100% - 28px),1240px);padding:22px 16px}.khae-calc-beta{right:10px;bottom:10px;min-width:96px;min-height:38px;font-size:.55rem}}
@media print{.khae-calc-mentor,.khae-calc-beta{display:none!important}}
`;
...
function mentorHref(){
  ...
  const q=new URLSearchParams({course:"Calculus I",concept:"calculus"});
...
function contextText(){
  const c=contextFromPage(),parts=["Calculus I"];
  if(c.week) parts.push(`Week ${c.week}`); if(c.view) parts.push(c.view); if(c.heading&&c.heading!==c.view) parts.push(c.heading);
  return parts.join(" · ");
}
function createMentor(){
  if(document.getElementById("khaeCalcMentor"))return;
  const main=document.querySelector("main");if(!main)return;
  const s=document.createElement("section");
  s.className="khae-calc-mentor";s.id="khaeCalcMentor";s.setAttribute("aria-labelledby","khaeCalcMentorTitle");
  s.innerHTML=`
  <div class="khae-calc-portrait"><img src="https://artist1970.github.io/Archaemenes.github.io/assets/Archaemens-high.png" alt="Archaemenes, High School educational mentor"></div>
  <div class="khae-calc-copy">
    <div class="khae-calc-kicker">Scholar · Educational Mentor · Mathematics</div>
    <h2 id="khaeCalcMentorTitle">Study Calculus with Archaemenes.</h2>
    <p>Use Archaemenes for conceptual explanations, hints, fresh examples, graph interpretation, modelling support, and help organizing reasoning before you return to independently graded work.</p>
    <p class="khae-calc-context" id="khaeCalcContext"></p>
    <div class="khae-calc-actions">
      <a class="primary" id="khaeCalcMentorLink" href="${mentorHref()}">Open Archaemenes</a>
      <a class="secondary" href="${ALGEBRA_URL}">Review Algebra I</a>
      <a class="secondary" href="${HIGH_URL}">Khaemenes Sr. High</a>
    </div>
    <p class="khae-calc-note">Mentor boundary: Archaemenes may teach, explain, guide and review. Calculus I course code remains authoritative for assessments, mastery, records and progression.</p>
  </div>`;
