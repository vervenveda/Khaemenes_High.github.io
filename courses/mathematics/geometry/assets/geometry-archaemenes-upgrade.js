...
/*
  Khaemenes Open-Age Geometry & Spatial Design
...
.hero h1{
  ...
  font-size:clamp(3.8rem,7vw,7.2rem)!important;font-weight:500!important;line-height:.93!important;letter-spacing:-.04em!important
}
...
.khae-geo-portrait{width:168px;height:168px;margin:auto;overflow:hidden;border:1px solid #d4b672;border-radius:50%;background:radial-gradient(circle,#fff8e6,#ead7ad)}
.khae-geo-portrait img{width:100%;height:100%;object-fit:cover;object-position:center top}
.khae-geo-copy{text-align:left}
.khae-geo-kicker{color:var(--k-burgundy);font:600 .67rem var(--k-formal);letter-spacing:.16em;text-transform:uppercase}
.khae-geo-mentor h2{margin:5px 0 8px;color:var(--k-forest);font:500 clamp(1.9rem,4vw,3.1rem)/1 var(--k-formal)}
.khae-geo-mentor p{margin:0;color:var(--k-muted)}
.khae-geo-context{margin-top:12px!important;color:var(--k-forest)!important;font-weight:700}
.khae-geo-actions{margin-top:18px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}
.khae-geo-actions a{min-height:43px;display:flex;align-items:center;justify-content:center;padding:9px 13px;border:1px solid var(--k-line);border-radius:8px;font-size:.68rem;font-weight:800;letter-spacing:.07em;text-transform:uppercase;text-decoration:none}
.khae-geo-actions .primary{color:#fff;background:var(--k-forest)}.khae-geo-actions .secondary{color:var(--k-forest);background:#fff}
.khae-geo-note{margin-top:13px!important;font-size:.78rem!important;color:#7a817d!important}
.khae-geo-beta{position:fixed;right:18px;bottom:18px;z-index:1300;min-width:112px;min-height:42px;display:flex;align-items:center;justify-content:center;padding:9px 15px;border:1px solid #cbaa64;border-radius:999px;color:#2c2618!important;background:linear-gradient(145deg,#f5e6c0,#d9b66a);box-shadow:0 11px 28px rgba(18,34,28,.16);font:600 .61rem var(--k-formal);letter-spacing:.11em;text-transform:uppercase;text-decoration:none}
@media(max-width:980px){.khae-geo-actions{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:860px){.khae-geo-mentor{grid-template-columns:1fr;text-align:center}.khae-geo-copy{text-align:center}.khae-geo-actions{grid-template-columns:1fr}}
@media(max-width:620px){.hero h1{font-size:clamp(3rem,13vw,4.8rem)!important}.khae-geo-mentor{width:min(calc(100% - 28px),1240px);padding:22px 16px}.khae-geo-beta{right:10px;bottom:10px;min-width:96px;min-height:38px;font-size:.55rem}}
@media print{.khae-geo-mentor,.khae-geo-beta{display:none!important}}
`;

const style=document.createElement("style");
style.id="khaemenes-geometry-archaemenes-theme-v1";
...
function mentorHref(){
  ...
  const q=new URLSearchParams({course:"Geometry",concept:"geometry and spatial reasoning"});
...
function contextText(){
  const c=contextFromPage(),parts=["Geometry"];
  if(c.week)parts.push(`Week ${c.week}`);if(c.view)parts.push(c.view);if(c.heading&&c.heading!==c.view)parts.push(c.heading);
  return parts.join(" · ");
}
function createMentor(){
  if(document.getElementById("khaeGeoMentor"))return;
  const main=document.querySelector("main");if(!main)return;
  const s=document.createElement("section");
  s.className="khae-geo-mentor";s.id="khaeGeoMentor";s.setAttribute("aria-labelledby","khaeGeoMentorTitle");
  s.innerHTML=`
  <div class="khae-geo-portrait"><img src="https://artist1970.github.io/Archaemenes.github.io/assets/Archaemens-high.png" alt="Archaemenes, High School educational mentor"></div>
  <div class="khae-geo-copy">
    <div class="khae-geo-kicker">Scholar · Educational Mentor · Mathematics</div>
    <h2 id="khaeGeoMentorTitle">Study Geometry with Archaemenes.</h2>
    <p>Use Archaemenes for proof strategy, constructions, spatial reasoning, transformations, coordinate geometry, modelling, and help interpreting diagrams without handing over graded work.</p>
    <p class="khae-geo-context" id="khaeGeoContext"></p>
    <div class="khae-geo-actions">
      <a class="primary" id="khaeGeoMentorLink" href="${mentorHref()}">Open Archaemenes</a>
      <a class="secondary" href="${ALGEBRA1_URL}">Review Algebra I</a>
      <a class="secondary" href="${ALGEBRA2_URL}">Explore Algebra II</a>
      <a class="secondary" href="${HIGH_URL}">Khaemenes Sr. High</a>
    </div>
    <p class="khae-geo-note">Mentor boundary: Archaemenes may teach, explain, guide and review. Geometry course code remains authoritative for assessments, mastery, records and progression.</p>
  </div>`;
  main.prepend(s);refresh();
}
function refresh(){const c=document.getElementById("khaeGeoContext"),l=document.getElementById("khaeGeoMentorLink");if(c)c.textContent=contextText();if(l)l.href=mentorHref()}
function addBeta(){if(document.getElementById("khaeGeoBeta"))return;const a=document.createElement("a");a.id="khaeGeoBeta";a.className="khae-geo-beta";a.href=BETA_URL;a.textContent="Beta Program";a.setAttribute("aria-label","Open the Verve N Veda Beta Program");document.body.appendChild(a)}
