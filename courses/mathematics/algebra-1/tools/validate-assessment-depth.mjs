...
for(const rel of [...weeklyParts.map(p=>path.basename(p)),path.basename(weeklyPatch),"weekly-mastery-engine-v2.js"])ok(weeklyPage.includes(rel),`weekly mastery page loads ${rel}`);
const weeklyEngine=read("assessments/assets/weekly-mastery-engine-v2.js");ok(weeklyEngine.includes("MASTERY=80")&&weeklyEngine.includes("REQUIRED=8"),"weekly engine declares the 80% / 8-of-10 contract");ok(weeklyEngine.includes("course_grade_eligible:w.graded===true"),"weekly engine preserves readiness versus graded evidence");ok(weeklyEngine.includes("dimension_scores"),"weekly engine records assessment-dimension evidence");ok(read("index.html").includes("assessments/weekly-mastery.html"),"course home exposes Weekly Mastery Center");

// Units 01-09: self-contained modern unit architecture.
...
// Units 10-13: shared curriculum bank + dedicated deterministic formal-mastery engine.
...
const sw=read("service-worker.js"),legacySw="service-worker-precache-v1.js";ok(exists(legacySw),"legacy full-course precache source is preserved");ok(sw.includes("khaemenes-algebra1-v3-release-hardening"),"service worker cache advances to release-hardening v3");ok(sw.includes("service-worker-precache-v1.js")&&sw.includes("legacyFiles"),"service worker rebuilds the preserved broad precache list");for(const name of ["weekly-mastery.html","weekly-mastery-engine-v2.js","exam-depth-v2.js","exam-engine.js","legacy-mastery-engine-v2.js","record-engine.js","records/course-completion-certificate.html"])ok(sw.includes(name),`release cache explicitly includes ${name}`);

if(failures){console.error(`\nAlgebra I assessment-depth validation failed: ${failures} problem(s).`);process.exit(1)}
console.log("\nAlgebra I assessment-depth validation passed: 36 weekly checks / 360 unique prompts, Units 01-09 fixed mastery, Units 10-13 lesson-balanced formal mastery, mixed-evidence cumulative mastery, evidence-gated completion, and release-hardened offline coverage.");
