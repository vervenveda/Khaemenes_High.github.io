...
const sw=read("service-worker.js"),legacySw="service-worker-precache-v1.js";ok(exists(legacySw),"legacy full-course precache source is preserved");ok(sw.includes("khaemenes-algebra1-v4-strict-80-gates"),"service worker cache advances to strict-gate v4");ok(sw.includes("service-worker-precache-v1.js")&&sw.includes("legacyFiles"),"service worker rebuilds the preserved broad precache list");for(const name of ["weekly-mastery.html","weekly-mastery-engine-v2.js","exam-depth-v2.js","exam-engine.js","legacy-mastery-engine-v2.js","record-engine.js","records/course-completion-certificate.html","strict-mastery-gates.js","course-progression-gates.js"])ok(sw.includes(name),`release cache explicitly includes ${name}`);

if(failures){console.error(`\nAlgebra I assessment-depth validation failed: ${failures} problem(s).`);process.exit(1)}
console.log("\nAlgebra I assessment-depth validation passed: 36 weekly checks / 360 unique prompts, Units 01-09 fixed mastery, Units 10-13 lesson-balanced formal mastery, mixed-evidence cumulative mastery, evidence-gated completion, strict progression gates, and v4 offline coverage.");