...
ok(authority?.version==="1.3"&&authority?.threshold===80&&authority?.focused_assessment==="v1.2","canonical mastery authority exposes v1.3 / 80% / focused-assessment contract");
...
const focusedRel="assets/focused-assessment-v1.js",focused=exists(focusedRel)?read(focusedRel):"",authoritySource=read(authorityRel);
ok(exists(focusedRel),"focused one-question assessment controller exists");
ok(focused.includes('VERSION="1.2"')&&focused.includes("One problem at a time")&&focused.includes('"#questionHost","#questions"')&&focused.includes("rootCandidate"),"focused controller supports dynamic one-problem presentation across assessment roots");
ok(focused.includes('document.getElementById("submitButton")')&&focused.includes('document.getElementById("score")')&&focused.includes('document.getElementById("resetButton")')&&focused.includes('document.getElementById("reset")'),"focused controller preserves both shared and dedicated assessment controls");
ok(authoritySource.includes("focused-assessment-v1.js")&&authoritySource.includes("ensureFocusedAssessment"),"canonical authority loads focused assessment presentation on assessment routes");
ok(read("units/unit-01/assessment/mastery-check.html").includes("focused-assessment-v1.js"),"special Unit 01 mastery page explicitly loads focused assessment presentation");
...
const sw=read("service-worker.js"),legacySw="service-worker-precache-v1.js";ok(exists(legacySw),"legacy full-course precache source is preserved");ok(sw.includes("khaemenes-algebra1-v7-responsive-deep-lessons"),"service worker cache advances to responsive/deep/focused v7");ok(sw.includes("service-worker-precache-v1.js")&&sw.includes("legacyFiles"),"service worker rebuilds the preserved broad precache list");for(const name of ["weekly-mastery.html","weekly-mastery-engine-v2.js","exam-depth-v2.js","exam-engine.js","legacy-mastery-engine-v2.js","record-engine.js","records/course-completion-certificate.html","strict-mastery-gates.js","course-progression-gates.js","mastery-authority-v1.js","dedicated-unit-gates-v1.js","focused-assessment-v1.js","responsive-layout-v1.css"])ok(sw.includes(name),`release cache explicitly includes ${name}`);for(let unit=2;unit<=9;unit++){const id=String(unit).padStart(2,"0");ok(sw.includes(`unit${id}.js`)&&sw.includes(`unit${id}-core.js`),`release cache explicitly includes Unit ${id} bootstrap and preserved core`)}

if(failures){console.error(`\nAlgebra I assessment-depth validation failed: ${failures} problem(s).`);process.exit(1)}
console.log("\nAlgebra I assessment-depth validation passed: 36 weekly checks / 360 unique prompts, canonical Week 2-36 lesson gating, focused one-problem assessment presentation, Units 01-09 fixed mastery with direct-route protection, Units 10-13 lesson-balanced formal mastery with canonical prerequisites, mixed-evidence cumulative mastery, evidence-gated completion, and v7 responsive/deep/focused offline coverage.");