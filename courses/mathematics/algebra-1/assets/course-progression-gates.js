...
function addCapstoneRunway(){
 if(document.getElementById("capstonePlanningRunway"))return;
 const structure=document.getElementById("structure");
 const shell=structure?.querySelector(".shell,.wrap");
 const grid=structure?.querySelector(".structure-grid");
 if(!shell||!grid)return;
 const runway=document.createElement("article");
 runway.id="capstonePlanningRunway";
 runway.className="notice capstone-runway";
 runway.setAttribute("aria-labelledby","capstonePlanningRunwayTitle");
 runway.innerHTML=`<p class="eyebrow">Capstone Planning Runway · Weeks 33–35</p><h3 id="capstonePlanningRunwayTitle">Plan early. Earn mastery only in the formal Week 36 sequence.</h3><p>During Weeks 33–35, while completing Unit 12, learners may choose a modelling question, identify possible data or evidence sources, define variables and units, and sketch assumptions for the Unit 13 integrated modelling portfolio. This planning runway is intentionally non-graded and awards no lesson, weekly, unit, capstone, or Final mastery credit.</p><p><strong>Week 36 remains formal:</strong> Unit 13 lessons, the Week 36 weekly mastery check, the Unit 13 mastery check, portfolio/oral-defence evidence, and the cumulative Final retain their established mastery and review requirements.</p>`;
 grid.after(runway);
}
...
function start(){addCapstoneRunway();if(authority())return boot();const s=document.createElement("script");s.src=authoritySrc;s.async=false;s.onload=boot;s.onerror=()=>failClosed("The canonical Algebra I mastery record failed to load. No later-unit unlocks were granted.");document.head.appendChild(s)}
