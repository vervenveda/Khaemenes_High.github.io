...
const scripts=[];
if(unit>=1&&unit<=10) scripts.push(new URL(`unit-${String(unit).padStart(2,"0")}-content-upgrade.js`,base).href);
...
function loadAt(i){
 ...
 s.onload=()=>loadAt(i+1);
 s.onerror=()=>{console.warn("Algebra I optional unit content upgrade not available:",scripts[i]);loadAt(i+1)};
