...
const MASTERY=80;
const gateScript=typeof document!=="undefined"?document.currentScript:null;
const authoritySrc=gateScript?new URL("../../assets/mastery-authority-v1.js",gateScript.src).href:null;
function authority(){return window.KhaemenesAlgebra1MasteryAuthority||null}
function missingFor(id){
 const A=authority();if(!A)return ["Canonical Algebra I mastery evidence authority"];
 const missing=[];
 if(id==="KH-MATH-A1-MIDTERM-U01-U06"){
  for(let u=1;u<=6;u++)if(!A.unitMastered(u))missing.push(`Unit ${String(u).padStart(2,"0")} mastery`);
  for(let w=2;w<=18;w++)if(!A.weekMastered(w))missing.push(`Week ${String(w).padStart(2,"0")} mastery`);
 }else if(id==="KH-MATH-A1-FINAL-36W"){
  for(let u=1;u<=13;u++)if(!A.unitMastered(u))missing.push(`Unit ${String(u).padStart(2,"0")} mastery`);
  for(let w=2;w<=36;w++)if(!A.weekMastered(w))missing.push(`Week ${String(w).padStart(2,"0")} mastery`);
  if(!A.midtermMastered())missing.push("reviewed Midterm mastery");
 }
 return missing;
}
...
function startGate(){if(authority())return enforceExamGate();if(!authoritySrc)return;const s=document.createElement("script");s.src=authoritySrc;s.async=false;s.onload=enforceExamGate;s.onerror=enforceExamGate;document.head.appendChild(s)}
if(typeof document!=="undefined"&&typeof setTimeout==="function")setTimeout(startGate,0);
})();