(function(){
  ...
  function getProgress(){const p=read(KEYS.progress,{version:1,lessons:{},updatedAt:null});if(!p.lessons||typeof p.lessons!=='object')p.lessons={};return p;}
  function saveProgress(p){p.updatedAt=now();return write(KEYS.progress,p);}
  function getLesson(id){return getProgress().lessons[id]||{};}
  function markLessonReviewed(id,reviewed=true){const p=getProgress();const old=p.lessons[id]||{};p.lessons[id]={...old,reviewed:!!reviewed,reviewedAt:reviewed?now():null};saveProgress(p);return p.lessons[id];}
  ...
  function bindDailyReview(root=document){
    const id=document.body.getAttribute('data-lesson-id');if(!id)return;
    const form=root.querySelector('[data-daily-review]');if(!form)return;
    const feedback=root.querySelector('[data-daily-feedback]');const challenge=root.querySelector('[data-challenge-unlock]');const button=form.querySelector('[data-score-review]');
    const renderSaved=()=>{const r=getLesson(id).dailyReview;if(!r)return;if(feedback){feedback.textContent=`Best daily review: ${r.bestScore}%${r.passed?' — completion threshold met.':' — review and try again.'}`;feedback.classList.toggle('good',!!r.passed);feedback.classList.toggle('warn',!r.passed);}if(challenge)challenge.classList.toggle('hidden',!r.passed);};
    if(button)button.addEventListener('click',()=>{const qs=[...form.querySelectorAll('[data-answer]')];if(!qs.length)return;let correct=0;qs.forEach(q=>{const name=q.getAttribute('data-answer');const chosen=form.querySelector(`input[name="${CSS.escape(name)}"]:checked`);if(chosen&&chosen.value===q.getAttribute('data-correct'))correct++;});const score=Math.round(correct/qs.length*100);const r=recordDailyReview(id,score);if(feedback){feedback.textContent=`Score: ${score}% (${correct}/${qs.length}). ${r.passed?'Completion threshold met. Optional challenge unlocked.':'Review the lesson, then try the check again.'}`;feedback.classList.toggle('good',r.passed);feedback.classList.toggle('warn',!r.passed);}if(challenge)challenge.classList.toggle('hidden',!r.passed);announce(`Daily review score ${score} percent.`);});
    renderSaved();
  }
